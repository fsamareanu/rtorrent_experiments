...
sudo checkinstall -D "$CHECKINSTALL_OPTS" --pkgrelease="$PKG_RELEASE" --pkggroup="libs" --install=no -y
sleep 3
...
EOF
sudo checkinstall -D "$CHECKINSTALL_OPTS" --pkgrelease="$PKG_RELEASE" --requires="xmlrpc-c \(\>= $XMLRPC_VER-$PKG_RELEASE\\)" --pkggroup="libs" --install=no -y
sleep 3
...
EOF
sudo checkinstall -D "$CHECKINSTALL_OPTS" --pkgrelease="$PKG_RELEASE" --requires="libtorrent \(\>= $LIBTORRENT_VER-$PKG_RELEASE\),xmlrpc-c \(\>= $XMLRPC_VER-$PKG_RELEASE\\)" --pkggroup="net" --install=no -y
sleep 3
